...
/// add movements, confine movements, spawn enemies in rand position
fn main() -> Result<()> {
    App::new()
...
fn spawn_enemies(
    ...
    for _ in 0..NUMBER_OF_ENEMIES {
        let x_randow = random::<f32>() * window.width();
        let y_randow = random::<f32>() * window.height();

        commands.spawn((
            SpriteBundle {
                transform: Transform::from_xyz(x_randow, y_randow, 0.0),
